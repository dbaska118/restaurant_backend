...
public class UserRepositoryTest {
    ...
    @Test
    public void findByEmailWithLockTest(){
        User admin = new Admin("user@wp.pl", "password", "Jan", "Nowak");
        User admin2 = new Admin("admin@wp.pl", "password", "Tomasz", "Kowalski");
        ...
        Optional<User> user = userRepository.findWithLockByEmail(admin.getEmail());
        Assertions.assertTrue(user.isPresent());
        Assertions.assertEquals(user.get(), admin);

        user = userRepository.findWithLockByEmail("email@wp.pl");
        Assertions.assertTrue(user.isEmpty());
    }
    ...
    public void findAllByRoleTest(){
        ...
        List<User> userList = userRepository.findAllByEnabledTrueAndRoleInOrderByIdAsc(List.of("admin"));
        Assertions.assertEquals(1, userList.size());
        Assertions.assertEquals(userList.get(0), admin);

        admin.setEnabled(false);
        entityManager.persist(admin);
        userList = userRepository.findAllByEnabledTrueAndRoleInOrderByIdAsc(List.of("admin"));
